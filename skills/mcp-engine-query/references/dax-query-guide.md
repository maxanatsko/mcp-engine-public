...
- `run_query` `operation: "vertipaq"` protects every storage byte, semantic/storage row-count, and semantic-cardinality value while preserving structural ordinals, segment quantities, revision, and raw protocol codes. The nested `storageDiagnostics.metricStates` entries distinguish semantic DAX metrics from physical DMV metrics and explain `null` values.
