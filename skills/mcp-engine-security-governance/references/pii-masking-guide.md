...
Protected aggregate metadata uses the same session scalar as query cells so an unmasked statistic cannot reveal the scalar. Semantic and physical storage row counts, true column cardinalities, blank/total counts, and storage byte magnitudes are protected even when the associated source column is text or otherwise exempt from cell masking. Generated DOCX/PDF/HTML semantic-model report files are an explicit unmasked export boundary and preserve source model metadata and numeric values; MCP response payloads remain protected. Aggregate contract fields keep their declared numeric type and saturate at that type's finite boundary on overflow. Structural counts remain exact: query/preview rows returned, requested limits, storage segment/object quantities, ordinals, revision, raw protocol codes, and `column_stats.values.returned_count` describe response or protocol structure rather than a business magnitude.

Responses that project protected metadata include `numeric_metric_masking`. Its `strategy` is `session_scalar`; `protected_fields` identifies transformed fields, while `derived_fields` identifies scale-invariant ratios or percentages derived from protected magnitudes. Policy application is reported even when zero, rounding, or saturation leaves a value numerically unchanged. The scalar itself is never returned.
